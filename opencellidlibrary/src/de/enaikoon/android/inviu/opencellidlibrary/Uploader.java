...
public class Uploader implements Runnable
{
	...
	public Uploader(final Database db, final Context aContext, boolean silent) throws MalformedURLException
	{
		this.myDatabase = db;
	...
	public void run()
	{
		...
		try
		{
			...
			FileLog.writeToLog("Upload request URL: " + "http://" + IP + "/gpsSuiteCellId/measure/uploadCsv");
			...
			try
			{
				...
				if (max > 0)
				{
					while (next.hasNext())
					{
						...
						sb = new StringBuilder("lat,lon,mcc,mnc,lac,cellid,timestamp,signal level,userid,extrainfo,create_at,updated_at,speed,direction\n");
						while (t < maxt && next.hasNext())
						{
							...
							sb.append(format.format(meassurement.getLat())
									...
									+ "," // extraInfo
									+ myDatabase.getFirstMeassurement(meassurement.getCellid(), meassurement.getLac(), meassurement.getMcc(),
											meassurement.getMnc(), meassurement.getTimestamp()) + "," + meassurement.getTimestamp() + ","
									+ (int) meassurement.getSpeed() + "," + (int) meassurement.getBearing() + "\n");
						}
						...
						mpEntity.addPart("datafile", new InputStreamBody(new ByteArrayInputStream(sb.toString().getBytes()), "text/csv", exsistingFileName));
						...
						FileLog.writeToLog(logTag + ": " + response.getStatusLine().getStatusCode() + " - " + response.getStatusLine());
